...
let _localGame = 0;
let _gameServer;
let _gameClient = new GameClient(this);
...
class InputHandler {
    constructor(canvas, serverConnection) {
        this.serverConnection = serverConnection;
        //this.registerHandlers(canvas);
    }

    registerHandlers(canvas) {
        canvas.keyPressed = this.keyPressed;
        canvas.keyReleased = this.keyReleased;
    }
    ...
    setKey(keyCode, value) {
        ...
        console.log(data);
        this.serverConnection.emit('setKey', data);
    }
}
...
function setKey(keyCode, value) {
    ...
    if (_gameClient) {
        _gameClient.recordInput(keyCode, value);
    }
    if (_gameServer)
...
function setup() {
    ...
    _inputHandler = new InputHandler(this, _serverConnection);
